...
hoverCheck = function hoverCheck() {
  return document.querySelectorAll('.interactive-hovtrigger').forEach(function (item) {
    var target = item.getAttribute('data-target'),
        selector = item.getAttribute('data-sel'),
        detailsMode = item.getAttribute('data-detailsMode');
    if (item.onmouseenter) return;
    if (target == "this") target = item;
    if (target == "parent") target = item.parentNode;
    if (target == "grandparent") target = item.parentNode.parentNode;
    if (!target) target = document;
    ...
    item.onmouseleave = function () {
      flag = false;
      if (window.innerWidth < 1500) teamSwiper.autoplay.start();
      if (selector) return target.querySelectorAll(selector).forEach(function (item) {
        if (detailsMode && item.classList.contains('details')) {
          if (item.style.maxHeight) {
            item.style.removeProperty('max-height');
    ...
    if (!flag && window.innerWidth < 1500) teamSwiper.autoplay.start();
  });
};
...
;
var deflt = document.getElementById('default');
document.querySelectorAll('.hoverjs').forEach(function (item) {
  return item.addEventListener('mouseenter', function () {
    deflt.classList.add('hidden');
    document.getElementById(item.getAttribute('data-hoverid')).classList.add('hover');
  });
});
document.querySelectorAll('.hoverjs').forEach(function (item) {
  return item.addEventListener('mouseleave', function () {
    deflt.classList.remove('hidden');
